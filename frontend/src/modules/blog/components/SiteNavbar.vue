    ...
    <div class="container-page h-16 sm:h-[72px]">
      <div class="flex h-full items-center justify-between gap-4">
        ...
        <router-link to="/" class="flex min-w-0 items-center gap-3 shrink-0 group">
          <span class="relative grid place-items-center size-9 sm:size-10 rounded-xl bg-gradient-to-br from-[var(--brand)] via-[#9333ea] to-[var(--accent)] text-white font-bold text-sm shadow-[var(--shadow-md)] ring-1 ring-white/30 dark:ring-white/10 transition-transform duration-300 group-hover:scale-[1.04]">
            {{ initialChar }}
            <span class="absolute -inset-2 rounded-2xl bg-gradient-to-br from-[var(--brand)] to-[var(--accent)] opacity-0 blur-xl transition-opacity duration-500 group-hover:opacity-30"></span>
          </span>
        ...
        <div class="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          >
            ...
            <kbd class="text-[10px] px-1.5 py-0.5 rounded border border-[var(--border)] text-[var(--text-muted)] bg-[var(--bg-muted)]">⌘ K</kbd>
          </router-link>
      ...
      >
        <nav class="container-page py-3 grid gap-1">
          <router-link
            ...
            :key="item.to"
            :to="item.to"
            class="px-3 py-2.5 rounded-xl text-sm font-medium transition-colors"
...
import { Sun, Moon, Settings, Search, Menu, X } from 'lucide-vue-next'
