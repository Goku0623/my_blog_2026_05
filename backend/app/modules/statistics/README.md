...
print(f"在线用户: {dashboard_data.online_users}")
...
### 4. 依赖要求
- **在线用户统计**：依赖 chatroom 模块的 WebSocket 连接管理
